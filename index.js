...
app.get('/api/population/:country', (req, res, next) => {
    ...
    countryData = dataService.data[population].filter(country => country["Country Name"].toUpperCase().includes(countryName.toUpperCase()))
...
app.get('/api/carbondioxide/:country', (req, res, next) => {
    const countryName = req.params.country
    countryData = dataService.data[carbondioxide].filter(country => country["Country Name"].toUpperCase().includes(countryName.toUpperCase()))
    res.json(countryData)
})
